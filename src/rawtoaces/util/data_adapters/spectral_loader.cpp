...
#include <rawtoaces/core/spectral_data.hpp>
#include <rawtoaces/core/spectral_solver.hpp>
    ...
    const std::string &type, core::SpectralData &out )
{
    ...
    if ( is_valid_CT( type ) )
    {
        ...
        {
            float cct = std::stoi( type.substr( 0, type.length() - 1 ) );
    ...
    for ( auto &path: paths )
    {
        ...
        if ( strcasecmp( temp.illuminant.c_str(), type.c_str() ) == 0 )
        {
            out = temp;
            return true;
        }
